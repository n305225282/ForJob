...
//            self.emojiView.frame = CGRectMake(0, self.backView.height, kScreenWidth, bottomHeight);
//            self.frame = CGRectMake(0, kScreenHeight - TabbarSafeBottomMargin - self.backView.height - bottomHeight, kScreenWidth, self.backView.height + bottomHeight);
            self.frame = CGRectMake(0, kScreenHeight - TabbarSafeBottomMargin - self.backView.height, kScreenWidth, self.backView.height);
            self.videoInputView.hidden = NO;
            self.textView.hidden = YES;
            [self changeTableViewFrame];
        }];
    } else {
        self.videoInputView.hidden = YES;
...
- (void)changeFrame:(CGFloat)height {
    ...
    self.frame = CGRectMake(0, kScreenHeight - self.backView.height - _keyboardHeight, kScreenWidth, self.backView.height);
    //改变更多按钮、表情按钮的位置
...
- (void)keyboardWillShow:(NSNotification *)notification {
    ...
    [UIView animateWithDuration:duration delay:0 options:[notification.userInfo[UIKeyboardAnimationCurveUserInfoKey] integerValue] animations:^{
        self.frame = CGRectMake(0, endFrame.origin.y - self.backView.height, kScreenWidth, self.height);
        [self changeTableViewFrame];
    } completion:nil];
}
...
- (VideoInputView *)videoInputView {
    if (!_videoInputView) {
        _videoInputView = [[VideoInputView alloc] initWithFrame:CGRectMake(CGRectGetMaxX(self.emojiBtn.frame) + viewMargin, viewMargin, kScreenWidth - (CGRectGetMaxX(self.emojiBtn.frame) + viewMargin) * 2, viewHeight)];
        _videoInputView.hidden = YES;
        [self.backView addSubview:_videoInputView];
    }
